...
const ChartPreview = ({
  ...
  useEffect(() => {
    ...
    if (requiredVariables.length > 0) {
      let errorMessage = (
        <span>
          {'Обязательные переменные: необходимо сопоставить'}{' '}
          {requiredVariables
            .map((d, i) => <span key={i} className="font-weight-bold">{d.name}</span>)
            .reduce((prev, curr) => [prev, ' и ', curr])}
        </span>
      )
      ...
      let errorMessage = (
        <span>
          ...
          {multivaluesVariables
            .map((d) => (
              <>
                {'минимум'} <span className="font-weight-bold">{d.minValues}</span>{' '}
                {'измерений для'} <span className="font-weight-bold">{d.name}</span>
              </>
            ))
            .reduce((prev, curr) => [prev, ' и ', curr])}
          {'.'}
    ...
    for (let variable in mapping) {
      if (
        ...
        const errorMessage = `Несоответствие типов: нельзя сопоставить ${mapping[variable].mappedType} с ${variableObj.name}.`
    ...
    try {
      ...
      try {
        ...
        setError({ variant: 'danger', message: 'Ошибка диаграммы. ' + e.message })
        setRawViz(null)
      }
    } catch (e) {
      while (domRef.current.firstChild) {
