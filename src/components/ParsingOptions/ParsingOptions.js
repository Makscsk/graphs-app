...
const dataRefreshCaptions = {
  "url": "Обновить данные по URL",
  "sparql": "Обновить данные из запроса"
}
      ...
      <Col className={styles.parsingOptions}>
        ...
        {get(dataRefreshWorkers, get(props.dataSource, 'type', ''), null) && (
          ...
          >
            ...
            {get(dataRefreshCaptions, get(props.dataSource, 'type', ''), "Обновить данные")}
          </Button>
        )}

        <div className="divider mb-3 mt-0" />

        <b>{'ТРАНСФОРМАЦИЯ ДАННЫХ'}</b>
