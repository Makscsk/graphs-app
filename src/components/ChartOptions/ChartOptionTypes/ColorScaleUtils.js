...
function ResetBtn({ resetScale }) {
  return (
    <OverlayTrigger
      key="bottom"
      placement="bottom"
      overlay={<Tooltip id={`tooltip-top`}>{'Перезагрузить домен'}</Tooltip>}
    >
...
function InvertBtn({ invertScale }) {
  return (
    <OverlayTrigger
      key="bottom"
      placement="bottom"
      overlay={<Tooltip id={`tooltip-top`}>{'Инверсировать'}</Tooltip>}
    >
...
function LockBtn({ handleChangeLocked, locked }) {
  return (
    <OverlayTrigger
      key="bottom"
      placement="bottom"
      overlay={<Tooltip id={`tooltip-top`}>{'Шкала'} {locked ? 'разблокировки' : 'блокировки'}</Tooltip>}
    >
      <span
        type="button"
